...
Physical TESTING attempt 2 did not execute that mode-5 transaction. On this OEM build, `am help` prints both required options but exits 255; the previous helper incorrectly treated the help exit as a capability failure. The same capture also proved that nested Activity `task=TaskRecord` text could overwrite an already recovered top component. The corrected helper treats exact advertised flags as preflight evidence, records the non-authoritative help exit, classifies the actual launch result separately and accepts success only after task/display/mode/bounds readback. Mode 5 and physical composition/input remain unqualified until the corrected APK is exercised.

...
final class ExperimentalMapPolicy {
    ...
    static void setEnabled(Context context, boolean ignoredCheckedState) {
        ...
        String[] labels = {"Fullscreen only · safe fallback", "Leaflet comparator",
                "Raw freeform task · experimental", "Android PiP · experimental"};
        String[] values = {HomeNavigationSurfacePolicy.FULLSCREEN, HomeNavigationSurfacePolicy.LEAFLET,
                HomeNavigationSurfacePolicy.RAW_FREEFORM, HomeNavigationSurfacePolicy.ANDROID_PIP};
        String current = HomeNavigationSurfacePolicy.mode(context); int selected = 0;
        for (int i = 0; i < values.length; i++) if (values[i].equals(current)) selected = i;
        new AlertDialog.Builder(activity).setTitle("HOME navigation surface")
                .setSingleChoiceItems(labels, selected, (dialog, which) -> {
                    HomeNavigationSurfacePolicy.setMode(activity, values[which]); dialog.dismiss(); activity.recreate();

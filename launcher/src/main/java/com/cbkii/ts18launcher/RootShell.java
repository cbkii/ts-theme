...
final class RootShell {
    ...
    private static Result runInternal(String command, long shellTimeoutSeconds, long waitMillis) {
        ...
        MediaEventTrace.record("root.begin", "bounded request waitMs=" + waitMillis);
        ...
        try {
            ...
            if (!completed) {
                ...
                return traced(new Result(false, -1, appendStatus(output, "root command timed out")));
            }

            joinQuietly(drainer, 1000);
            return traced(new Result(true, process.exitValue(), output.toString()));
        } catch (IOException e) {
            return traced(new Result(false, -1, e.getClass().getSimpleName() + ": " + e.getMessage()));
    ...
    private static Result traced(Result result) {
        MediaEventTrace.record("root.end", classify(result));
        return result;
    }
    ...
    private static String classify(Result result) {
        ...
        if (!result.completed && result.output != null
                && result.output.toLowerCase(java.util.Locale.ROOT).contains("timed out")) {
            return "timeout";
        }
        if (!result.completed) return "unavailable-or-interrupted";
        return "rejected exit=" + result.exitCode;
    }

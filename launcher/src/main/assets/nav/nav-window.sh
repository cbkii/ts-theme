...
parse_task_snapshot() {
  ...
  awk -v pkg="$pkg" -v hint="$hint" '
    ...
    matched && /mBounds=Rect\(/ {
      task_bounds=bounds_of($0)
      next
    }
    matched && /\* Hist #0:/ {
      hist0=1
    ...
    matched && /(mSupportsPictureInPicture|supportsPictureInPicture)=/ {
      line=$0
      if (line ~ /(mSupportsPictureInPicture|supportsPictureInPicture)=true/) task_pip="1"
      else if (line ~ /(mSupportsPictureInPicture|supportsPictureInPicture)=false/) task_pip="0"
      next
    }
    matched {
      mode=mode_of($0)
      if (mode != "") task_mode=mode
    }
    END {
      finish()
      if (count == 0) {
      ...
      # Keep package-only acquisition fail-closed when more than one matching task exists.
      if (count>1&&hint=="0") {
...
for required in am dumpsys awk getprop pm settings; do
  ...
  freeform|verify-freeform|pip|verify-pip)
    ...
    if [ "$action" = freeform ]; then
      ...
      if [ "$WINDOWING_MODE" != 2 ]; then
        ...
        am stack move-top-activity-to-pinned-stack "$STACK_ID" "$expected" >/dev/null 2>&1 || \
      ...
      am stack resize "$STACK_ID" "$expected" >/dev/null 2>&1 || \

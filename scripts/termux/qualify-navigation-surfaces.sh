...
if [ "$MODE" = raw_freeform ] || [ "$MODE" = android_pip ]; then
    ...
    case "$line" in
      OK*)
        ...
        if [ "$display" = 0 ] && [ "$wm" = "$expected" ] && printf '%s' "$component" | grep -q "^$TARGET/"; then
          result PASS task-state "mode=$wm display=$display component=$component"
        else
  ...
  result PASS task-state "mode=$MODE has no foreign embedded-task requirement"
fi

if [ "$ROOT_OK" -eq 1 ] && { [ "$MODE" = raw_freeform ] || [ "$MODE" = android_pip ]; }; then
  screenshot before-map-input
  if timeout -k 2 5 su -c 'input swipe 650 360 700 360 250' >/dev/null 2>&1; then
    result INFO map-gesture 'input dispatched; compare screenshots'
  else

...
- Add a staged standalone Android 10/API 29 HOME launcher lane (`com.cbkii.ts18launcher`) alongside the legacy DoFun theme: direct TS18 geometry, local/OSM WebView map, event-driven MediaSession music, separate radio authority, four configurable quick-launch slots with Navigation/Radio/Music/Bluetooth fallbacks, an in-HOME collapsible app drawer, provider-aware navigation handoff, reversible HOME selection, bounded Magisk-root install/rollback/measurement helpers, APK-envelope CI and a separately qualified prerelease-candidate workflow. The OSM tile layer is now reused during same-tile pan/GPS updates rather than rebuilt on every render. DoFun remains enabled as recovery until physical reboot/cold-boot/ACC and vehicle-function validation passes.

...
- Replace the previous parser experiment with a controlled DoFun compatibility candidate derived from the user-supplied working `sfp_ts20.apk` and the exact-host `kp.jar` contract: top-level media/radio now uses the host-proven `medias` widget type, while `local_music`/`local_radio`/`bt_music` remain media-source modes rather than invented widget types.
- Add project-authored `assets/.gen/c.json` capability/index metadata, a mirrored `assets/layout-1280x652/` land asset set, and an explicit 1280x652 theme-local coordinate model that projects back onto the established physical 1280x720 / 55 px top-inset geometry.
- Make AAPT2 intentionally package `.gen/c.json` and fail the release contract if generated/1280x652 compatibility assets are missing. The candidate keeps the unique package/plug-in identity, independent signing, one-DEX/no-native contract and current toolchain so the parser vocabulary/metadata/viewport variables can be tested before changing D8 or adding any vendor SDK code.
- Record the exact SFP_TS20 reference hash and clean-room structural findings without committing vendor APK bytes, resources, executable code or signing material.

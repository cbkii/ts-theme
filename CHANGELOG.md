...
- Replace the SFP_TS20-only parser experiment with a controlled cross-reference compatibility carrier based on all supplied working themes, especially the minimal SFP_FYD18/SFP_T5 envelope: restore the proven top-level `local_radio` and `local_music|bt_music` widgets and remove `.gen/c.json` / 1280x652 mirror assumptions from the physical-test candidate.
- Add an isolated `compat/legacy-d8/` build lane pinned to Gradle 5.1.1 + AGP 3.4.0 / D8 1.4.77, with a genuine `land` flavor, generated `BuildConfig`, DoFun-style `<versionCode>_<YYMMDDHHMMSS>.land` versionName, minimal framework-icon manifest, and a stable 14-digit project `themeId` using the evidence-backed `oemId=1132`.
- Gate the legacy carrier in CI on one DEX/no native ABI, RePlugin `Entry`, the eight-class legacy `library/*` topology, absence of modern D8 synthetic classes, D8 1.4.77 metadata, intended import metadata, and V1+V2 signing; preserve the exact CI-signed carrier as a short-lived physical-test artifact while keeping the normal release pipeline unchanged until TS18 validation succeeds.
- Record a clean-room matrix of the stock SFP_TS10, TS10_TW, SFP_FYD18, SFP_T5 and SFP_TS20 working references, including the findings that `.gen`, `variety.theme`, unified `medias`, and `oemId=190` are not universal requirements; no vendor APK bytes, resources, executable code or signing material are committed.

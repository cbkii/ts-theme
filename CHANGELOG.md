...
- Bundle a read-only Magisk `service.d` activation diagnostic that records all discoverable DoFun/RePlugin registrations, working-theme runtime paths, activation-time logs, derived plug-in/cache paths and mount namespaces. It runs at most twice with a 170-second hard cap per run and exports upload-ready ZIPs to `Download`.
